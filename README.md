...
3. 摄像头权限、视频帧、识别模型分开管理；预览和识别使用同一份已解码画面，增加黑屏/断流检测、超时、取消及迟到资源清理。
...
15 项浏览器回归覆盖实际 MediaPipe Worker 的初始化及推理、模式布局一致性、前景抽牌与释放、手势动作、权限/设备错误、取消、黑屏恢复、摄像头切换、断流、Worker 异常/超时、后台释放和三种移动视口。动作与故障测试使用合成输入，不能替代各设备真实摄像头和真人手势验收。
